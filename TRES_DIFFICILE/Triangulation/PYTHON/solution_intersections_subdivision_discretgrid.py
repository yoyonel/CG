...
class Grid:
    ...
    def clip_by_line(self, p0, p1):
        """
        p0 et p1 positions analytiques dans la grille initiale
        :param p0: surement le centre de la zone courante
        :param p1: position dansW la grille initiale
        :return:
        """
        #
        # p0: inside grid
        ...
        def func_filter(intersection):
            ...
            if intersection:
                ...
                result = (dot >= 0) & (dot <= dot2_p0p1)
            return result

        list_intersections = filter(func_filter, list_intersections)

        return () if not list_intersections else list_intersections[0]
    ...
    def clip_from_center(self, p):
        ...
        #
        try:
            return self.clip_by_line(self.center, p)[0]
        except:
            return ()
    ...
    def compute_position_for_splitting(self, last_pos):
        ...
        point_symmetry = 2 * self.center - last_pos
        new_pos = point_symmetry
