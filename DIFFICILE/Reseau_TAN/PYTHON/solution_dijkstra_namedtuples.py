...
class Graph(object):
    ...
    def add_edge(self, _id_node1, _id_node2):
        """ """
        self.edges.setdefault(_id_node1, []).append(_id_node2)

    def del_edge(self, _id_node1, _id_node2):
        """ """
        self.edges[_id_node1].remove(_id_node2)
...
def dijkstra_search(graph, _start, _goal):
...
def reconstruct_path_cost(_came_from, _cost_so_far, _start, _goal):
    current = _goal
    path = [current]
    cost = _cost_so_far[current]
    while current != _start:
...
graph_routes = GraphForTAN(
    dict_stations)  # on transmet le dict des stops pour le calcul des poids/distances sur les stops
for i in xrange(m):
    route = raw_input()
    #
