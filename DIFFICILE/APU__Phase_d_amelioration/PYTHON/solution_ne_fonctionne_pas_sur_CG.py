...
class Node:
    ...
    def update_valency(self):
        """
        """
        # on compte le nombre d'indice -1 (non connexion)
        # et on inverse ce nombre
    ...
    def remove_node(self, node, id_node):
        ...
        for dir, id_neighboor in node.dict_neighboors.iteritems():
            ...
            if id_neighboor != -1:
                ...
                #self.remove_potential_link(id_neighboor, self.get_inv_dir(dir))
...
class APU_IA(Network):
    ...
    def apply_rule_4(self):
        """
        Rule 4: on est bloque et il reste des liens,
        on tente une connexion "au pif" :p
        On cherche le node avec la somme la plus eleve de connexions (amount de connexion)
        """
    ...
    def apply_rule_5(self):
        """
        Rule 5: On cherche des nodes actifs en amount 1 qui ont des connexions possibles isolantes (voisin en amount 1 egalement)
        ps: c'est une version simple de la regle (faudrait generaliser avec 2 et empecher les connexions 2=2)
        """
        for id_node in self.list_ids_active_nodes:
            node = self.get_node(id_node)
            #print >> sys.stderr, "id_node: %d, %d %d" % (id_node, node.get_amount(), node.get_valency())
            #if (node.get_amount() == 1) & (node.get_valency() > 1):
            if node.get_amount() == 1:
                #print >> sys.stderr, ""
                #print >> sys.stderr, "######## rule 5 #######"
                #print >> sys.stderr, ""
                b_updated_neighboors = False
                for dir, id_neighboor in node.dict_neighboors.iteritems():
                    node_neighboor = self.get_node(id_neighboor)
                    if node_neighboor.get_amount() == 1:
                        #print >> sys.stderr, "rule 5 -> id_node=%d - node = %s " % (id_node, node)
                        self.remove_potential_link(id_node, dir)
                        b_updated_neighboors = True
                        #self.remove_potential_link(id_neighboor, self.get_inv_dir(dir))
                if b_updated_neighboors:
                    node.update_valency()
...
# - solution multiples -> initier un move si pas de regles de resolutions applicables au depart [DONE]
# - probleme de topologie (voisins qui ne sont pas voisins) sur le dernier test 'expert' !!! bizarre ...
#
# probleme avec la regle 5, faut la repenser, voir repenser toutes les regles :-/
apu_ia.resolve()
