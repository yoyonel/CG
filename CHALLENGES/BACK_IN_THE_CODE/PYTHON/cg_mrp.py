...
def intersect(a, b):
    ...
    :param a: Range
    :param b: Range
    :return: Renvoie l'intersection de a et b
...
class MapForFloodFilling:
    ...
    def valid_coordinates(self, col, row):
        #
        return (col >= 0) & (col < self.l) & (row >= 0) & (row < self.h)
    ...
    def get_tile_type(self, col, row):
        #
        return self.get_tile(col, row)[0]

    def get_tile_id(self, col, row):
        #
        return self.get_tile(col, row)[1]

    def validate_coordinates(self, col, row):
        #
        return (col, row) if self.valid_coordinates(col, row) else None

    def tile_has_id(self, col, row):
        #
        return self.get_tile_id(col, row) != 0

    def set_id(self, col, row, id):
        #
        self.tiles[row][col] = (self.tiles[row][col][0], id)
        #
        self.area_for_id[id] += 1

    def get_neighboors_coords(self, col, row):
        #
        list_neighboors_coords = (
            self.validate_coordinates(col - 1, row),
            self.validate_coordinates(col + 1, row),
            self.validate_coordinates(col, row - 1),
            self.validate_coordinates(col, row + 1)
        )
        return filter(lambda coord: coord, list_neighboors_coords)

    def get_valid_neighboors_coords_with_no_id(self, col, row):
        #
        list_neighboors_tiles = self.get_neighboors_coords(col, row)
    ...
    def get_area(self, col, row):
        #
        return self.area_for_id[self.get_tile_id(col, row)]
...
class GameStrategy(GameState):
    ...
    def __init__(self, *args):
        ...
        self.last_game_round = self.game_round
    ...
    def next_move(self):
        ...
        next_position = self.board.center
        try:
            if self.last_game_round > self.game_round:
                # il y a eu un back in time
                # on reset la fsm et on revient au state 0 init
                self.state = 0
                ...
                freeElements = [pos[::-1] for pos in self.board.get_cells_available()]
                ...
                # on determine le rectangle 'libre' proche de la position courante
                ...
                if openRects:
                    ...
                    rect_selected = self.choose_rect_in_list_openRects(list(openRects))

                    self.cur_rect = rect_selected
                    self.select_a_new_destination_from_rect(rect_selected)
                    self.state = 1
                    next_position = self.next_move()
                else:
                    self.state = 0
            ...
            # ya eu un pb (quelconque inconnu) on revient au state = 0 (INIT)
            self.state = 0
        finally:
            self.last_position = self.me.position
    ...
    def choose_rect_in_list_openRects(self, list_openRects):
        ...
        '''
        max_distance = float(self.board.l + self.board.h)
        max_area = float(self.board.l * self.board.h)
        coef_for_distance = 0.25
        coef_for_area = 0.75
        list_openRects.sort(
            key=lambda rect:
            (max_area / float(self.board.distance(
                self.me.position,
                self.board.compute_nearest_position_on_border_for_rect(self.me.position, rect)
            ))) * coef_for_distance +
            (float(rect.area()) / max_area) * coef_for_area
        )
